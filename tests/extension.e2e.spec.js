...
// =============================================================================
// NAVIGATION AND CLICK BEHAVIOR TESTS
// =============================================================================

test.describe("Navigation: Click behavior for windows, groups, and tabs", () => {
  test("clicking on a tab should focus the window and activate the tab", async ({ page }) => {
    await setupPageWithMockData(page, richMockData);
    ...
    await page.evaluate(() => {
      window._apiCalls = [];
      const originalWindowsUpdate = window.chrome.windows.update;
      const originalTabsUpdate = window.chrome.tabs.update;

      window.chrome.windows.update = (windowId, options) => {
        window._apiCalls.push({ api: 'windows.update', windowId, options });
        return originalWindowsUpdate(windowId, options);
      };

      window.chrome.tabs.update = (tabId, options) => {
        window._apiCalls.push({ api: 'tabs.update', tabId, options });
        return originalTabsUpdate(tabId, options);
      };
    });

    // Expand window and group to access tabs
    const firstWindow = page.locator(".window-item").first();
    await firstWindow.locator(".expand-icon").first().click();
    const firstGroup = firstWindow.locator(".group-item").first();
    await firstGroup.locator(".expand-icon").first().click();
    ...
    const firstTab = firstGroup.locator(".tab-item").first();
    await firstTab.click();

    // Verify API calls were made
    const calls = await page.evaluate(() => window._apiCalls);
    expect(calls).toHaveLength(2);
    expect(calls[0]).toMatchObject({ api: 'windows.update', windowId: 1, options: { focused: true } });
    expect(calls[1]).toMatchObject({ api: 'tabs.update', tabId: 1, options: { active: true } });
  });

  test("clicking on window header (not expand icon) should focus the window", async ({ page }) => {
    await setupPageWithMockData(page, richMockData);
    ...
    // Click on the window title text (not the expand icon)
    const windowHeader = page.locator(".window-header").first();
    const windowTitleSpan = windowHeader.locator("span:last-child");
    await windowTitleSpan.click();

    // Verify window focus API was called
    const calls = await page.evaluate(() => window._apiCalls);
    expect(calls).toHaveLength(1);
    expect(calls[0]).toMatchObject({ api: 'windows.update', windowId: 1, options: { focused: true } });
  });

  test("clicking on group header (not expand icon) should focus first tab in group", async ({ page }) => {
    await setupPageWithMockData(page, richMockData);

    // Expand window to see groups
    const firstWindow = page.locator(".window-item").first();
    await firstWindow.locator(".expand-icon").first().click();
    ...
    // Click on the group title (not the expand icon)
    const groupHeader = firstWindow.locator(".group-header").first();
    const groupTitleSpan = groupHeader.locator("span:last-child");
    await groupTitleSpan.click();

    // Verify both window focus and tab activation were called
    const calls = await page.evaluate(() => window._apiCalls);
    expect(calls).toHaveLength(2);
    expect(calls[0]).toMatchObject({ api: 'windows.update', windowId: 1, options: { focused: true } });
    expect(calls[1]).toMatchObject({ api: 'tabs.update', tabId: 1, options: { active: true } });
...
test.describe("Empty and Error States", () => {
  test("should display 'No windows found' when there are no windows", async ({ page }) => {
    const emptyMockData = { windows: [], groups: [] };
    await setupPageWithMockData(page, emptyMockData);

    await expect(page.locator(".empty-msg")).toContainText("No windows found");
    await expect(page.locator(".window-item")).toHaveCount(0);
  });

  test("should display 'Error loading windows' when API fails", async ({ page }) => {
    await page.goto(`file://${popupPath}`);
    ...
    await page.addInitScript(() => {
      ...
      window.chrome = {
        windows: {
          getAll: () => Promise.reject(new Error("API Error")),
          update: () => Promise.resolve(),
          onCreated: createEventMock(),
          onRemoved: createEventMock(),
        },
    ...
    await expect(page.locator(".empty-msg")).toContainText("Error loading windows");
  });
});
...
test.describe("Fallback Values and Edge Cases", () => {
  test("should display 'New Tab' for tabs without a title", async ({ page }) => {
    const noTitleMockData = {
      windows: [
        {
          id: 1,
          title: "Test Window",
          tabs: [
            { id: 1, title: "", groupId: -1, favIconUrl: null },
            { id: 2, title: null, groupId: -1, favIconUrl: null },
          ],
        },
      ],
      groups: [],
    ...
    await setupPageWithMockData(page, noTitleMockData);

    const windowItem = page.locator(".window-item").first();
    await windowItem.locator(".expand-icon").first().click();

    const tabItems = windowItem.locator(".tab-item");
    await expect(tabItems.nth(0)).toContainText("New Tab");
    await expect(tabItems.nth(1)).toContainText("New Tab");
  });

  test("should display '(Untitled Group)' for groups without a title", async ({ page }) => {
    const noGroupTitleMockData = {
      windows: [
        {
          id: 1,
          title: "Test Window",
      ...
      groups: [{ id: 1, title: "", color: "blue", windowId: 1 }],
    };

    await setupPageWithMockData(page, noGroupTitleMockData);

    const windowItem = page.locator(".window-item").first();
    await windowItem.locator(".expand-icon").first().click();

    const groupHeader = windowItem.locator(".group-header").first();
    await expect(groupHeader).toContainText("(Untitled Group)");
  });

  test("should display 'Window {id}' for windows without a title", async ({ page }) => {
    const noWindowTitleMockData = {
      windows: [
        {
          id: 42,
          title: "",
          tabs: [{ id: 1, title: "Tab 1", groupId: -1, favIconUrl: null }],
        },
      ],
      groups: [],
    };

    await setupPageWithMockData(page, noWindowTitleMockData);

    const windowHeader = page.locator(".window-header").first();
    await expect(windowHeader).toContainText("Window 42");
  });

  test("should display favicon when favIconUrl is provided", async ({ page }) => {
    await setupPageWithMockData(page, richMockData);
    ...
    // Check that favicon img elements exist
    const tabIcons = firstGroup.locator(".tab-item .tab-icon");
    await expect(tabIcons).toHaveCount(2);
    await expect(tabIcons.first()).toHaveAttribute("src", "https://example.com/favicon.ico");
  });

  test("should not display favicon when favIconUrl is null", async ({ page }) => {
    const noFaviconMockData = {
      windows: [
        {
          id: 1,
          title: "Test Window",
          tabs: [{ id: 1, title: "Tab Without Favicon", groupId: -1, favIconUrl: null }],
        },
      ],
      groups: [],
    };

    await setupPageWithMockData(page, noFaviconMockData);

    const windowItem = page.locator(".window-item").first();
    await windowItem.locator(".expand-icon").first().click();

    const tabItem = windowItem.locator(".tab-item").first();
    await expect(tabItem.locator(".tab-icon")).toHaveCount(0);
  });
});
...
test.describe("Help Modal Behavior", () => {
  test("should close help modal when clicking outside (on backdrop)", async ({ page }) => {
    await setupPageWithMockData(page, richMockData);
    ...
    // Click on the modal backdrop (the modal element itself, not its content)
    await page.evaluate(() => {
      const modal = document.getElementById("help-modal");
      modal.click(); // Simulates clicking on the backdrop
    });
...
test.describe("Expansion State Persistence", () => {
  test("should preserve window expansion state after refreshUI", async ({ page }) => {
    await setupPageWithMockData(page, richMockData);
    ...
    // First window should still be expanded
    const firstWindowAfter = page.locator(".window-item").first();
    await expect(firstWindowAfter).toHaveClass(/expanded/);
  });

  test("should preserve group expansion state after refreshUI", async ({ page }) => {
    await setupPageWithMockData(page, richMockData);
    ...
    const firstGroupAfter = page.locator(".window-item").first().locator(".group-item").first();
    await expect(firstGroupAfter).toHaveClass(/expanded/);
  });
});
...
test.describe("Group Color Mapping", () => {
  const allColorsMockData = {
    windows: [
      {
        id: 1,
        title: "Color Test Window",
        tabs: [
          { id: 1, title: "Grey Tab", groupId: 1, favIconUrl: null },
          { id: 2, title: "Blue Tab", groupId: 2, favIconUrl: null },
          { id: 3, title: "Red Tab", groupId: 3, favIconUrl: null },
          { id: 4, title: "Yellow Tab", groupId: 4, favIconUrl: null },
          { id: 5, title: "Green Tab", groupId: 5, favIconUrl: null },
          { id: 6, title: "Pink Tab", groupId: 6, favIconUrl: null },
          { id: 7, title: "Purple Tab", groupId: 7, favIconUrl: null },
          { id: 8, title: "Cyan Tab", groupId: 8, favIconUrl: null },
          { id: 9, title: "Orange Tab", groupId: 9, favIconUrl: null },
        ],
      },
    ],
    groups: [
      { id: 1, title: "Grey Group", color: "grey", windowId: 1 },
      { id: 2, title: "Blue Group", color: "blue", windowId: 1 },
      { id: 3, title: "Red Group", color: "red", windowId: 1 },
      { id: 4, title: "Yellow Group", color: "yellow", windowId: 1 },
      { id: 5, title: "Green Group", color: "green", windowId: 1 },
      { id: 6, title: "Pink Group", color: "pink", windowId: 1 },
      { id: 7, title: "Purple Group", color: "purple", windowId: 1 },
      { id: 8, title: "Cyan Group", color: "cyan", windowId: 1 },
      { id: 9, title: "Orange Group", color: "orange", windowId: 1 },
    ],
  };

  test("should correctly map all 9 Chrome tab group colors", async ({ page }) => {
    await setupPageWithMockData(page, allColorsMockData);
    ...
    const expectedColors = {
      ...
      blue: "rgb(26, 115, 232)",    // #1a73e8
      red: "rgb(217, 48, 37)",      // #d93025
      yellow: "rgb(249, 171, 0)",   // #f9ab00
      green: "rgb(24, 128, 56)",    // #188038
      pink: "rgb(208, 24, 132)",    // #d01884
      purple: "rgb(161, 66, 244)", // #a142f4
      cyan: "rgb(0, 123, 131)",     // #007b83
      orange: "rgb(250, 144, 62)",  // #fa903e
    };

    const groupDots = windowItem.locator(".group-dot");
    ...
    for (const [name, rgb] of Object.entries(expectedColors)) {
      expect(colors).toContain(rgb);
    }
  });

  test("should use grey as fallback for unknown colors", async ({ page }) => {
    const unknownColorMockData = {
      windows: [
        {
          id: 1,
          title: "Test Window",
          tabs: [{ id: 1, title: "Tab", groupId: 1, favIconUrl: null }],
        },
      ],
      groups: [{ id: 1, title: "Unknown Color Group", color: "invalid_color", windowId: 1 }],
    };

    await setupPageWithMockData(page, unknownColorMockData);

    const windowItem = page.locator(".window-item").first();
    await windowItem.locator(".expand-icon").first().click();

    const groupDot = windowItem.locator(".group-dot").first();
    const color = await groupDot.evaluate((el) =>
      window.getComputedStyle(el).backgroundColor
    );

    // Should fall back to grey (#5a5a5a = rgb(90, 90, 90))
    expect(color).toBe("rgb(90, 90, 90)");
  });
});
